...
args = { 'ip': '', 'u': '', 'p': '', 'f': 'scripts/import.xml', 't': 'IDRAC', 's': 'Graceful', 'e': 'On'}
...
def import_SCP_local_filename(args):
    idrac_ip = args['ip']
    idrac_username = args['u']
    idrac_password = args['p']
    filename = args['f']
...
def Run():
    check_supported_idrac_version(args)
    import_SCP_local_filename(args)
